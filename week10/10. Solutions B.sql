...
SELECT e.firstName, e.lastName, SUM(
...
SELECT p.productName, p.productLine, SUM(
        od.quantityOrdered * od.priceEach
    ) AS totalSales
FROM
    products p
...
SELECT MAX(creditLimit) AS "Max credit limit", MIN(creditLimit) AS "Min credit limit", SUM(creditLimit) AS "Total credit limit", AVG(creditLimit) AS "Avg credit limit"
FROM customers
WHERE
    country IS NOT NULL
...
SELECT f.rating AS "Film rating", COUNT(f.film_id) AS "Number of films"
FROM
    film f
...
SELECT d.dept_name, e.first_name, e.last_name, DATEDIFF(
        MAX(de.to_date), MIN(de.from_date)
    ) AS duration_days
FROM
    departments d
    JOIN dept_emp de ON d.dept_no = de.dept_no
...
SELECT d.dept_name, SUM(s.salary) AS totalExpenditure
FROM
    departments d
    JOIN dept_emp de ON d.dept_no = de.dept_no
...
SELECT t.title, COUNT(e.emp_no) AS totalEmployees, AVG(s.salary) AS avgSalary
FROM
    titles t
